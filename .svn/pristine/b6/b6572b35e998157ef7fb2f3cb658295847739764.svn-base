...
var TableInit = function() {
    ...
    oTableInit.Init = function() {
        $('#tb_departments').bootstrapTable({
            url: port + '/v1/assetAcceptance/assetAcceptanceIntoDepot',
            method: 'get',
            locale: 'zh-CN',
            ...
            columns: [{
                ...
                formatter: function(value, row, index) {
                    var result = "<a href='javascript:;' class='info'>入库</a>";
                    return result;
                }
            }],
            ...
            queryParams: function(params) {
                var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
                    ...
                    applyPersonNumber: $('#applyJobNum').val(), //工号
    ...
    'click .info': function(e, value, row, index) {
        ...
        layer.open({
            ...
            yes: function(index, layero) {
                var body = layer.getChildFrame('body', index);
                var dataGrid = window[layero.find('iframe')[0]['name']].getAllData();
                ...
                layer.open({
                    ...
                    btnAlign: 'c',
                    area: ['350px', '220px'],
                    content: '<div style="text-align:center;margin-top: 30px;">确定入库？</div>',
                    yes: function(index, layero) {
                        $.ajax({
                            url: port + '/v1/assetAcceptance/saveIntoDepot',
                            type: 'post',
                            data: {
                                intoDepotId: row.id,
                                pJobNum: localStorage.getItem('pJobnum'),
                                iasList: JSON.stringify(dataGrid) //所有数据转字符串
                            },
                            success: function(res) {
                                if (res > 0) {
                                    $("#tb_departments").bootstrapTable('refresh');
